...
function dispatch(buffer, meta, line) {
        ...
        if (
                buffer.charCodeAt(0) === 62 && buffer.charCodeAt(1) === 62 &&
                buffer.charCodeAt(2) === 62
        ) {
                console.log("STDOUT detected");
                return handler.stdout(buffer.slice(3).trim());
        } else if (
                buffer.charCodeAt(0) === 60 && buffer.charCodeAt(1) === 60 &&
        ...
        if (meta.pivot) {
                ...
                if (meta.lattice) {
                        ...
                        if (values.length === 2) {
                                if (
                                        values[1].charCodeAt(0) === 60 &&
                                        values[1].charCodeAt(1) === 60 &&
                                        values[1].charCodeAt(2) === 60
                                ) {
                                        handler.assignment(
                                                values[0],
                                                values[1],
